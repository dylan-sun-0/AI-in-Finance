...
smoother = FunctionTransformer(pandas_ewm_smoother, kw_args={'span': 30})
#smoother = FunctionTransformer(talib_ewm_smoother, kw_args={'span': 30})
...
spans_rs = [{'span': 6}, {'span': 10}, {'span': 20}, {'span': 50}]
...
This code applies an EWMA to each column in x_train_in, using the pandas_ewm_smoother function, 
which smooths the data using a specified span (defaulting to 10)
This made the training a lot less noisy and the graphs were smoother
...
Smoothing Prices looked better
Applying EWMA to prices helps to create a stable, noise-reduced price series, 
which, in turn, can produce smoother and potentially more accurate returns when calculated.
