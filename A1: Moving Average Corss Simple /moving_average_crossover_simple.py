...
try:
    ...
    data.to_csv(address, header = True, index=True, encoding='utf-8')
except Exception:
    msg = "yahoo problem"
    dfP = pd.DataFrame()
...
plt = dfP[['Close','42d','252d']].plot(grid=True,figsize=(8,5))
plt.figure.savefig('Closing and Moving Averages.png')
...
dfP['syst_rets'] = dfP['pct_rets'] * (dfP['Stance'].shift(1))
dfP['syst_cum_rets'] = (1 + dfP['syst_rets']).cumprod() - 1 + 1
dfP['mkt_cum_rets'] = (1 + dfP['pct_rets']).cumprod() - 1 + 1

plt = dfP[['mkt_cum_rets','syst_cum_rets']].plot(grid=True,figsize=(8,5)) #plotting returns percent cumul
plt.figure.savefig('Market and System Returns.png')
