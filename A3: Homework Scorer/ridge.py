...
df['retFut1'] = df['<OPEN>'].pct_change(periods=1).shift(-1) #if you enter the trade right after the open
#df = np.log(df+1)
...
#plt.show()
plt.savefig(r'%s.png' %("Spearman Correlation Matrix"))
...
plot_corr(corr, size=20)
#plt.show()
plt.savefig(r'%s.png' %("Pearson Correlation Matrix"))
...
a_rs = np.logspace(-7, 0, 20, endpoint=True)
...
plt.xlabel('Date')
#plt.show()
plt.savefig(r'%s.png' %("TestCumulative"))
...
#plt.show()
plt.savefig(r'%s.png' %("Coefficients"))
