...
df['retFut1_categ'] = 0
df.loc[df['retFut1'] <= df['retFut1'].quantile(q=0.33), 'retFut1_categ'] = -1
...
talib = talib = ColumnTransformer(
    transformers=[('rsiadx', rsiadx, ['<CLOSE>', '<HIGH>', '<LOW>', '<OPEN>'])],
...
"""
svc = NuSVC(class_weight='balanced') #to balance the label categories, if they do not occur in equal numbers
svc = NuSVC() 
