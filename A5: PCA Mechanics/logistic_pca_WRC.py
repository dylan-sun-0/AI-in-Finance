...
1. Is is ok to use PCA on categorical features as they are?
Genearlly not, since it's meant for continuous data, categorization you'd want to use a classifier 

2. Does scaling apply before PCA?
Yes, you should apply scaling before PCA, in the code StandardScaler is used first to ensure a mean of 0

3. Does it make sense to apply PCA in combination with ridge regression?
Yes, PCA reduces the dimensions of data, then ridge can regularizes it more to avoid overiftting, 
however this depends on data, it is not always necessary 

4. Are the features selected by PCA a subset of the original features?
not technically, since PCA creates linear combinations of original features 
these new components are uncorrelated and ordered by the amount of variance they capture
so they may differ significantly

5. Are the features being used to train the logistic regression in the pipeline
generated by pca.fit_transform(X[, y]) or pca.inverse_transform(X)?
The logistic regression in this code is trained on features generated by pca.fit_transform(X[, y])

6. Does PCA eliminate multicollinearity?
Yes, PCA reduces multicollinearity by transforming correlated features into a set of uncorrelated components

7. Does L1 or L2 regularization correct multicollinearity also?
Both L1 (lasso) and L2 (ridge) regularization can help mitigate the impact of multicollinearity. 
L2 regularization reduces multicollinearity by shrinking coefficients, while L1 can set some coefficients to zero,
from here: https://towardsdatascience.com/l1-and-l2-regularization-methods-ce25e7fc831c
