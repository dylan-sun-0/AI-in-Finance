...
def information_coefficient_select(X, y):
    ...
    for i in range(X.shape[1]):
        rho_arr[i], pval_arr[i] = spearmanr(X[:, i], y)
...
detrended_open = detrendPrice.detrendPrice(openp.iloc[10000:12000])
detrended_retFut1 = detrendPrice.detrendPrice(df.iloc[10000:12000].retFut1).fillna(0)
...
for selectors: mutual info regression can capture non linear relationships
f_regression is univariate so it considers only linear relationships 

for scorer's: R squared is a strictly linear relationship, so it can't predict non linear trends
Spearman's uses a monotonic function and can be non linear 

For the p_value in WRC, you want a small value here to "reject null hypothesis" 
say p_value < 0.05 so you can reject the null 
This means that the data is better than just random predictions
